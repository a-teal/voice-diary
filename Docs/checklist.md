...
### 3.7 버그 수정 ✅
- [x] STT 끊김 문제 수정
  - 원인: onend 핸들러에서 React 클로저로 인한 stale state
  - 해결: isRecordingRef 사용하여 실시간 상태 추적
- [x] 타임존 문제 수정
  - 원인: toISOString()이 UTC 기준이라 날짜가 하루 전으로 저장됨
  - 해결: getLocalDateString() 함수로 로컬 타임존 기준 저장
- [x] 녹음 모달 UI 개선
  - 버튼 위치 조정 (더 위로)
  - 아이콘/텍스트 크기 축소 (컴팩트)
