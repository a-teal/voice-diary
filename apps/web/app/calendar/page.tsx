    ...
    <div className="flex flex-col min-h-screen bg-slate-50">
      ...
      <header className="px-6 pt-12 pb-8 bg-white shadow-sm flex justify-between items-center">
        ...
        <h2 className="text-xl font-bold text-slate-900">
          {format(currentMonth, i18n.language === 'ko' ? "yyyy년 M월" : "MMMM yyyy", { locale })}
        </h2>
        <button
      ...
      <div className="flex-1 p-4 overflow-y-auto pb-24">
        ...
        <div className="grid grid-cols-7 mb-4">
          {(t('date.weekdays', { returnObjects: true }) as string[]).map((day, idx) => (
            <div key={idx} className="text-center text-xs font-semibold text-slate-400 uppercase tracking-wider">
              {day}
            </div>
          ))}
        </div>
        ...
        <div className="mt-6 bg-white rounded-2xl shadow-sm border border-slate-100 p-4">
          <h3 className="text-sm font-medium text-slate-500 mb-3">{t('calendar.monthSummary')}</h3>
